...
mod tests {
    ...
    async fn test_list_users_pagination() {
        ...
        for i in 0..5 {
            let user = User::new(format!("user{}", i), format!("user{}@example.com", i), format!("hash{}", i));
            db.insert_user(&user).await.unwrap();
        }
    ...
    async fn test_list_repositories_by_owner() {
        ...
        let user1 = User::new("owner1".to_string(), "owner1@example.com".to_string(), "hash1".to_string());
        let user2 = User::new("owner2".to_string(), "owner2@example.com".to_string(), "hash2".to_string());
